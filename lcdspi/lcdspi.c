...
*****************************************************************************************/
void lcd_print_char(int rID, int fc, int bc, char c, int orientation) {
	...
	if (orientation == ORIENT_NORMAL) regions[rID].current_x += width * scale;
}
...
void display_put_c(int rID, char c) {
	...
	if (c >= MainFont[2] && c < MainFont[2] + MainFont[3]) {
		if (CURRENT_X + gui_font_width > regions[rID].end_x) {
			display_put_c(rID, '\r');
			display_put_c(rID, '\n');
		}
		if (CURRENT_Y + gui_font_height> regions[rID].end_y) {	// If it would print down, DONT
	...
	switch (c) {
		case '\b':
			...
			if (CURRENT_X < regions[rID].start_x)	//Go to end of previous line
			{
				regions[rID].current_y -= gui_font_height;	//Go up one line
				
				if(CURRENT_Y < regions[rID].start_y) regions[rID].current_y = 0;
				regions[rID].current_x = (((regions[rID].end_x-regions[rID].start_x)/FONT_WIDTH) - 1) * gui_font_width;  //go to last character
			}			
			return;
		case '\r':
		...
		case '\n':
			regions[rID].current_x = 0;
			regions[rID].current_y += (regions[rID].is_printing_downward) ? gui_font_height : -gui_font_height;
			/*
			...
			// Change of plans
			if (CURRENT_Y > regions[rID].end_y) {
				lcd_reset_coords(rID);
			}
			return;
		case '\t':
...
char lcd_put_char(int rID, char c, int flush) {
	lcd_putc(rID, 0, c);
...
void lcd_reset_coords(int rID) {
	regions[rID].current_x = regions[rID].start_x;
	regions[rID].current_y = regions[rID].start_y;
}
...
void lcd_region_clear(int rID) {
	draw_rect_spi(regions[rID].start_x, regions[rID].start_y, regions[rID].end_x - 1, regions[rID].end_y - 1, gui_bcolour);
}
...
int lcd_region_create(int start_x, int start_y, int end_x, int end_y, uint8_t print_downwards)
{
	...
	// Bounds checking
	if(start_x<0||start_x>hres) { return -3; }
	if(start_y<0||start_y>vres) { return -4; }
	if(end_x<0||end_x>hres) { return -5; }
	if(end_y<0||end_y>vres) { return -6; }
...
void lcd_putc(int rID, uint8_t devn, uint8_t c) {
	display_put_c(rID, c);
}
