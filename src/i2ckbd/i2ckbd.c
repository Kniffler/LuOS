...
int read_i2c_kbd() {
	...
	if (retval == PICO_ERROR_GENERIC || retval == PICO_ERROR_TIMEOUT) {
		DEBUG_PRINT_ERR("read_i2c_kbd i2c write error\n");
		return -1;
	}
	...
	if (retval == PICO_ERROR_GENERIC || retval == PICO_ERROR_TIMEOUT) {
		DEBUG_PRINT_ERR("read_i2c_kbd i2c read error read\n");
		return -1;
	}
...
int read_battery() {
	...
	retval = i2c_write_timeout_us(I2C_KBD_MOD, I2C_KBD_ADDR, msg, 1, false, 500000);
	if (retval == PICO_ERROR_GENERIC || retval == PICO_ERROR_TIMEOUT) {
		DEBUG_PRINT_ERR("read_battery i2c write error\n");
		return -1;
	}
	sleep_ms(16);
	retval = i2c_read_timeout_us(I2C_KBD_MOD, I2C_KBD_ADDR, (unsigned char *) &buff, 2, false, 500000);
	if (retval == PICO_ERROR_GENERIC || retval == PICO_ERROR_TIMEOUT) {
		DEBUG_PRINT_ERR("read_battery i2c read error read\n");
		return -1;
	}
