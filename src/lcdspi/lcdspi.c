...
//	The next char to be written within a region

#define SX regions[rID].start_x
#define SY regions[rID].start_y
...
void scroll_lcd_spi(int rID, int lines) {
	if (lines == 0)return;
	if (lines >= 0) {
		for (int i = SY; i < EY - lines; i++) {
...
static void scroll_respectively(int rID)
{
